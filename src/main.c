...
#include "../include/bruxo.h"
#include "../include/pocao.h"
...
int Start() {
  ...
  if (StartWitchers() != 1){
    printf("\nErro ao allocar a memoria dos bruxos!\n");
    code = 0;
  }
  if (StartPatiants() != 1){
    printf("\nErro ao allocar a memoria dos pacientes!\n");
    code = 0;
  }
  if (StartPotions() != 1) {
    printf("\nErro ao allocar a memoria das pocoes!\n");
    code = 0;
  }
  if (StartTreatments() != 1) {
    printf("\nErro ao allocar a memoria dos tratamentos!\n");
    code = 0;
  }
...
void WitchersList() {
  ...
  for (i = 0; i < GetQtyWitcher(); i++) {
    ...
    printf("\n %d - %s - %s", witcher.code, witcher.name, witcher.especiality);
  }
}
...
void PotionsList() {
  ...
  for (i = 0; i < GetQtyPotion(); i++) {
    ...
    printf("\n %d - %s - %s", potion.code, potion.name, potion.type);
  }
}
...
void PatiantsList() {
  ...
  for (i = 0; i < GetQtyPatiant(); i++) {
    ...
    printf("\n %d - %s - %d - %.2f ", patiant.code, patiant.name, patiant.age,
           patiant.height);
  }
  printf("\n");
}
...
void TreatmentListPacient() {
  ...
  printf("\n\nCode - Nome Bruxo - Nome Pocao - Nome Paciente - Duracao - "
         "Dosagem ");
  for (i = 0; i < GetQtyTreatment(); i++) {
    Treatment treatment;
    ...
    if (treatment.witcherCode == code)
      printf("\n %d - %s - %s - %s - %d - %d ", treatment.code, witcher.name,
             potion.name, patiant.name, treatment.duration, treatment.dosage);
  }
  printf("\n");
}
...
void TreatmentRegistration() {
  ...
  if (treatment.code < 0 ||
      GetTreatmentByCode(treatment.code, &treatment) == 1) {
    printf("\nCodigo invalido!");
    return;
  }
