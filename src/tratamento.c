...
int StartTreatments() {
  treatmentFile = fopen("./data/TratamentoArq.txt", "r+b");

  if (treatmentFile == NULL) {
    treatmentFile = fopen("./data/TratamentoArq.txt", "w+b");
...
int TreatmentRemove(Treatment treatment) {
  ...
  remove("./data/TratamentoArq.txt");

  fclose(temp);

  rename("./data/temp.txt", "./data/TratamentoArq.txt");
...
int GetQtyTreatment() { return qtyTreatments; }
