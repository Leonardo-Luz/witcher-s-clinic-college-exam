...
int StartWitchers() {
  witcherFile = fopen("./data/BruxoArq.txt", "r+b");

  if (witcherFile == NULL) {
    witcherFile = fopen("./data/BruxoArq.txt", "w+b");
...
int WitcherRemove(Witcher witcher) {
  ...
  remove("./data/BruxoArq.txt");

  fclose(temp);

  rename("./data/temp.txt", "./data/BruxoArq.txt");
...
int GetQtyWitcher() { return qtyWitchers; }
