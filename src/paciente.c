...
int StartPatiants() {
  patiantFile = fopen("./data/PacienteArq.txt", "r+b");

  if (patiantFile == NULL) {
    patiantFile = fopen("./data/PacienteArq.txt", "w+b");
...
int PatiantRemove(Patiant patiant) {
  ...
  remove("./data/PacienteArq.txt");

  fclose(temp);

  rename("./data/temp.txt", "./data/PacienteArq.txt");
...
int GetQtyPatiant() { return qtyPatiants; }
