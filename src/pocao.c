...
int StartPotions() {
  potionFile = fopen("./data/PocaoArq.txt", "r+b");

  if (potionFile == NULL) {
    potionFile = fopen("./data/PocaoArq.txt", "w+b");
...
int PotionRemove(Potion potion) {
  ...
  remove("./data/PocaoArq.txt");

  fclose(temp);

  rename("./data/temp.txt", "./data/PocaoArq.txt");
...
int GetQtyPotion() { return qtyPotions; }
